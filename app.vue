<template>
  ...
  <Body
    ...
    <div id="outerCursor" aria-hidden="true"
      class="z-[9999] transition-all duration-[250ms] ease-out fixed -translate-x-1/2 -translate-y-1/2 size-20 rounded-full border border-black dark:border-white pointer-events-none"
      :class="{ 'mix-blend-difference bg-white size-32': isHovered }"
      :style="{ left: cursorPosition.x + 'px', top: cursorPosition.y + 'px' }" />
    <div id="innerCursor" aria-hidden="true"
      class="z-[9998] fixed -translate-x-1/2 -translate-y-1/2 size-2 rounded-full bg-black dark:bg-white pointer-events-none"
      :style="{ left: cursorPosition.x + 'px', top: cursorPosition.y + 'px' }" />
    ...
    <div class="min-h-dvh flex flex-col justify-between px-4 pt-8 pb-6 sm:px-12">
      <header class="max-w-6xl mx-auto w-full mb-20 md:mb-32 lg:mb-40 text-black dark:text-white">
        <nav class="flex justify-between items-center">
          <NuxtLink to="/">
            <svg aria-hidden="true" class="h-8 w-auto fill-black dark:fill-white" width="101" height="54"
              viewBox="0 0 101 54" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
